...
TELEGRAM_BOT_TOKEN = '7874377360:AAE2awPw4g4YOyP2oL7o1pjtek8FREGr5u0'
ADMIN_USER_ID = 6171974438
MONGO_URI = "mongodb+srv://Kamisama:Kamisama@kamisama.m6kon.mongodb.net/"
DB_NAME = "IGL"
COLLECTION_NAME = "users"
attack_in_progress = False
ATTACK_TIME_LIMIT = 180  # Maximum attack duration in seconds
COINS_REQUIRED_PER_ATTACK = 5  # Coins required for an attack
ATTACK_COOLDOWN = 180  # Cooldown period in seconds (5 minutes)
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        "*🎉 Welcome to the Ultimate UDP Flooder! 🎉*\n\n"
        ...
        "*💬 Queries or Issues? 💬*\n"
        "*Contact Admin: @Surya_Cheat_Owner*"
    )
...
async def legacy(update: Update, context: CallbackContext):
    ...
    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Incorrect usage! Use: /legacy <add|rem> <user_id> <coins>*", parse_mode='Markdown')
        return
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user["coins"] < COINS_REQUIRED_PER_ATTACK:
        await context.bot.send_message(
            chat_id=chat_id,
            text="*💰 Insufficient coins! Please contact the admin to acquire more coins. DM: @Surya_Cheat_Owner*",
            parse_mode='Markdown'
        )
        return
...
async def help(update: Update, context: CallbackContext):
    ...
    message = (
        ...
        "- *If you encounter any issues, contact the admin: @Surya_Cheat_Owner*\n\n"
