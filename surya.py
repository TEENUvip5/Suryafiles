...
TOKEN = "7874377360:AAE2awPw4g4YOyP2oL7o1pjtek8FREGr5u0"
...
def handle_start(message):
    if message.chat.type == 'group':
        ...
        welcome_message = f"""
*स्वागत है  𝐒𝐔𝐑𝐘𝐀 𝐕𝐈𝐏 𝐃𝐃𝐎𝐒   में!* 🚀  
@Surya_Cheat_Owner से अपडेट्स के लिए जुड़ें।
...
- /when - बचे हुए अटैक समय की जानकारी लें  
"""
...
def handle_attack_command(message):
    ...
    if bot.attack_in_progress:
        bot.send_message(message.chat.id, "सबर करो भोसड़ी के BGMI  को चोदने में बिजी हूं")
        return

    bot.send_message(message.chat.id, "💣 BGMI की गांड़ मारने के लिये तैयार \n"
...
def process_attack_command(message):
    try:
        ...
        bot.send_message(message.chat.id, "⚠️ If you do not provide feedback, you may be banned from the group. plz provide feedback @Surya_Cheat_Owner")
...
def handle_when_command(message):
    if not bot.attack_in_progress:
        bot.send_message(message.chat.id, "अभी आप attack मार सकतें है।")
    else:
        elapsed_time = loop.time() - bot.attack_start_time
        remaining_time = max(0, bot.attack_duration - elapsed_time)
        bot.send_message(message.chat.id, f"अभी attack चल रहा है: {int(remaining_time)} सेकंड।")
